...
def dsvi_logreg_full(Y, X, mu=None, C=None, niter=100, rho=1e-3,
                     m0=None, S0=None):
    """ DSVI for Bayesian logistic regression with automatic relevance
        determination.
        ...
        C  : p-vector, diagonal variational covariance
        niter : number of iterations in this round
        m0 : p-vector, prior mean of parameters
        S0 : p-vector, prior covariance of parameters

        Returns:
          - mu : p-vector, mean vector of variational approximation
          - C  : p-vector, diagonal variational covariance
          - ELBO : values of ELBO
    """

    print "This function has not been tested!"
    ...
    # Step size constant in this round
    ...
    for it in xrange(niter):
        ...
        dmu = grad_logreg - mu / Cmu
        dC = grad_logreg * z + 1. / C - C / Cmu

        mu += rho * dmu
        C += 0.1*rho * dC
        C[C <= 1e-4] = 1e-4  # Numerical stability
